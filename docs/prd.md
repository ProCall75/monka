...
> **Références :** [Architecture](architecture.md) — [Glossaire](glossary.md) — [SPRINT.md](../SPRINT.md)
...
| Data clinique éparpillée | 165 questions, 235 règles, 369 micro-tâches dans des fichiers MD dispersés |
...
|---|----------|--------------------|---------|
| O1 | Moteur clinique déterministe fonctionnel | 100% des 235 règles évaluables en temps réel | Sprint |
| O2 | Architecture isolée et maintenable | 0 fichier > 300L, séparation engine/UI stricte | Sprint |
...
| Temps d'évaluation | < 100ms pour 235 règles | Performance runtime |
...
```
 ...
 │  321 coefficients        │    + Seuil par V (faible/modéré/élevé/critique)
 │  score = Σ(coef × rép)  │
 └──────────┬───────────────┘
            │
            ▼
 ...
 │  235 règles (JSONB)      │    + MPs activés (24 possibles)
 │  AND-logic conditions    │    + Niveau: standard | ccc | critique
 │  9 opérateurs            │
 └──────────┬───────────────┘
            │
            ▼
 ...
 └──────────┬───────────────┘
            ...
            ▼
 ┌──────────────────────────┐
 │  MICRO-TÂCHES            │──→ Actions concrètes (369 possibles)
...
| Règles d'activation | 235 | `activation_rules` | `id` |
| Questions scoring | 321 | `scoring_questions` | `id` |
| Seuils scoring | 20 | `scoring_thresholds` | `id` |
| Recommandations | 198 | `recommendations` | `id` |
| Micro-Tâches (MT) | 369 | `micro_taches` | `id` |
| Questions de suivi | 30 | `suivi_questions` | `id` |
| Content Blocks | variable | `content_blocks` | `id` |
| CR Templates | variable | `cr_templates` | `id` |
...
│   │   ├── activation_rules (235)    FK: category_id, mp_id
│   │   ├── recommendations (198)      FK: category_id, mp_id
│   │   └── micro_taches (369)         FK: category_id, mp_id
│   └── question_mp_mapping (155)      FK: question_id, mp_id
├── scoring_questions (321)  FK: question_id, vulnerability_id
├── scoring_thresholds (20)  FK: vulnerability_id
└── suivi_questions (30)     FK: vulnerability_id, mp_id
```

> Voir [architecture.md — §5 Schéma Base de Données](architecture.md#5-schéma-base-de-données) pour le diagramme ER complet.
...
|----------|-------|--------|
| Évaluation moteur (235 règles) | < 100ms | `performance.now()` dans engine |
| First Contentful Paint | < 1.5s (desktop), < 2.5s (mobile) | Lighthouse |
| Chargement données Supabase (13 tables) | < 2s | Network waterfall |
...
### Phase 2 — Richesse Clinique (Blocs 4-8) — En cours

| Bloc | Livrable | Statut |
|:----:|----------|:------:|
| 4 | Explications cliniques (sens_clinique, tooltips, content_blocks) | ⬜ |
| 5 | Vue Externe premium (patient-facing) | ⬜ |
| 6 | CR Médecin professionnel (export PDF) | ⬜ |
| 7 | Fiches Questions avec filtres avancés | ⬜ |
| 8 | Vulnérabilités Drill-Down (V→MP→Détail) + **QG before-deploy** | ⬜ |

### Phase 3 — Intelligence Clinique (Blocs 9-16)

| Bloc | Livrable | Statut |
|:----:|----------|:------:|
| 9 | Navigation + Documents Officiels | ⬜ |
| 10 | Score-Action Gap Detection | ⬜ |
| 11 | Chaîne Clinique Traçable (Q→R→Reco→MT) | ⬜ |
| 12 | Scoring Enrichi (décomposition visuelle) | ⬜ |
| 13 | Heatmap Couverture Clinique | ⬜ |
| 14 | Mode What-If | ⬜ |
| 15 | Comparaison Personas | ⬜ |
| 16 | Score de Confiance Moteur | ⬜ |

### Phase 4 — Production-Ready (Blocs 17-19)

| Bloc | Livrable | Statut |
|:----:|----------|:------:|
| 17 | Vérification Intégrité Données + **QG before-deploy** | ⬜ |
| 18 | Observabilité IDEC (dashboard professionnel) | ⬜ |
| 19 | Préparation Multilingue + **QG after-deploy (certification finale)** | ⬜ |
...
|---|--------|:-----------:|:------:|------------|
| R1 | SimulatorPage reste > 200L car trop de state partagé | Haute | Moyen | Accepté à ~945L pour v1 ; extraction progressive dans les hooks dédiés (blocs 4+) |
| R2 | Données cliniques incomplètes (wording manquant, sens_clinique vide) | Moyenne | Haut | Fallback UI ("Non documenté"), Engine Health Score alerte proactivement |
| R3 | Performance dégradée avec 235 règles × persona switch | Faible | Moyen | Évaluation < 100ms vérifiée ; `useMemo` partout |
| R4 | Supabase free tier limites (500MB, 50K rows) | Faible | Faible | ~2K rows actuellement ; monitoring dans Engine Health |
| R5 | Changement de structure clinique (ajout V6, modification règles) | Moyenne | Haut | Architecture data-driven : ajout V6 = ajout lignes DB, pas de code |
| R6 | Auth non préparée retarde le passage en production | Faible | Haut | Architecture déjà auth-ready (`clinical/hooks/` isolé) |
...
|---------|-------|-----------|
| **Supabase** `mbxeqrvofrmhqlwlefff` | PostgreSQL (13 tables), stockage données cliniques | [Dashboard](https://supabase.com/dashboard/project/mbxeqrvofrmhqlwlefff) |
